...
\todo[inline]{Share mode algorithm}
...
\todo{Adapt to current share mode algorithm}
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\Require{$B, T, S, D$}
		\Statex
		\State{$missing\_piece = 0$} \Comment{missing piece}
		\ForAll{$p \in connected\_peers$}
			\If{$p$ is a $leecher$ $and$ $p$ is not in share\_mode}	
				\State{$missing\_pieces$} += {$total\_pieces - pieces(p)$}
			\EndIf	
		\EndFor
		\State{$missing\_pieces$} -= {$B \times |connected\_seeders|$}		
		\If{$missing\_pieces \leq 0$}
			\State \Return
		\EndIf
		\If{$|connected\_seeders|$ / $|connected\_peers| > S$}
			\State disconnect excess seeders
		\EndIf
		\If{$downloaded \times T > uploaded$}
			\State \Return
		\EndIf
		\If{$downloading \times D > downloaded$}
			\State \Return
		\EndIf
		\If{$|connected\_peers| - min(pieces_rarities) < T$}
			\State \Return
		\EndIf
